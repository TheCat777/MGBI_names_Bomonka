...
void main()
{
    ...
    if (iTime > 16) {
        //col += distance(gl_FragCoord.xy, resolution.xy / 2) * (pow(2.71828, 0.5 * iTime - 8));
        col += (1 - (distance(gl_FragCoord.xy, resolution.xy / 2) / (resolution.x / 2) - 0.3)) * (pow(4*2.71828, 0.14 * iTime - 2.24) - 1);
    }
